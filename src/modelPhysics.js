...
const INITIAL_RESOLVE_STEPS = 48;
...
function createClipCollider(node, config) {
  ...
  return {
    ...
    center,
    boundaryRadius: center.length() + Math.hypot(visualRadius, visualHalfHeight),
...
function createBallCollider(node, config) {
  ...
  const visualRadius = sphere.radius * Math.max(
  ...
  return {
    descriptor: RAPIER.ColliderDesc.ball(visualRadius * config.ballColliderScale),
    center,
...
class ModelPhysics {
  constructor({ root, camera, canvas, config, sphereNode, movingNodes, staticNodes }) {
    ...
    this.physicsCenter = this.boundary.center.clone().applyMatrix4(this.boundary.matrix);
  ...
  createDynamicBody(node) {
    ...
    if (typeof body.setAdditionalSolverIterations === "function") {
      body.setAdditionalSolverIterations(this.config.additionalSolverIterations ?? 4);
    }

    colliderData.descriptor
      .setTranslation(colliderData.center.x, colliderData.center.y, colliderData.center.z)
    ...
    const delaySeed = hashName(`${node.name}:delay`);
    const flowSeed = hashName(`${node.name}:flow`);
    const dragSeed = hashName(`${node.name}:drag`);
    const driftSeed = hashName(`${node.name}:drift`);
    const driftAngle = driftSeed * Math.PI * 2;

    this.bodies.push({
      node,
      body,
      boundaryRadius: colliderData.boundaryRadius,
      flowDelay: THREE.MathUtils.lerp(0, this.fluid.maxBodyDelay, delaySeed),
      flowFactor: THREE.MathUtils.lerp(
        this.fluid.flowFactorMin,
        this.fluid.flowFactorMax,
        flowSeed,
      ),
      dragFactor: THREE.MathUtils.lerp(
        this.fluid.dragFactorMin,
        this.fluid.dragFactorMax,
        dragSeed,
      ),
  ...
  resolveInitialOverlaps() {
    ...
    for (let step = 0; step < INITIAL_RESOLVE_STEPS; step += 1) {
      this.world.step();
    ...
    this.world.gravity = originalGravity;
    for (const { body } of this.bodies) {
      body.resetForces(false);
      body.resetTorques(false);
      body.setLinvel({ x: 0, y: 0, z: 0 }, false);
  ...
  bindPointerEvents() {
    ...
    this.canvas.addEventListener("pointermove", (event) => {
      if (!event.isPrimary) return;
      if (event.pointerType !== "mouse" && event.pointerId !== this.activeTouchPointerId) {
        return;
      }
  ...
  handlePointerMove(event) {
    ...
    const velocityX = (event.clientX - this.lastPointer.x) /
      Math.max(rect.width, 1) / safeDeltaTime;
    const velocityY = -(event.clientY - this.lastPointer.y) /
      Math.max(rect.height, 1) / safeDeltaTime;

    const pointerSmoothing = THREE.MathUtils.clamp(this.config.pointerSmoothing, 0, 1);
    const smoothingAlpha = 1 - Math.pow(
      1 - pointerSmoothing,
  ...
  applyPointerVelocity(velocityX, velocityY) {
    ...
    if (!this.gestureArmed || gestureSpeed < GESTURE_THRESHOLD) return;

    this.gestureArmed = false;
    const maximumVelocity = Math.min(MAX_GESTURE_VELOCITY, this.config.maxPointerVelocity);
  ...
  startFluidMotion(strength, direction) {
    const flowWasActive = this.fluidEnergy > this.fluid.minimumEnergy;
    this.hasInteracted = true;
    this.fluidStrength = strength;
    this.fluidEnergy = Math.max(
      this.fluidEnergy * 0.65,
      THREE.MathUtils.lerp(0.88, 1.18, strength),
    );
    if (!flowWasActive) this.fluidAge = 0;
    this.flowSerial += 1;
  ...
  getFluidVelocity(item, position, allowFlow) {
    if (!allowFlow || this.fluidEnergy <= 0) return { x: 0, y: 0, z: 0 };

    const localAge = this.fluidAge - item.flowDelay;
    if (localAge <= 0) return { x: 0, y: 0, z: 0 };

    const ramp = THREE.MathUtils.smoothstep(localAge, 0, this.fluid.rampDuration);
    ...
    const wallAttenuation = THREE.MathUtils.lerp(1, 0.5, wallFactor);
    ...
    const energyRamp = this.fluidEnergy * ramp;
    const time = localAge * item.frequency + this.flowSerial * 0.67;
    const turbulence = this.fluid.turbulenceSpeed *
      item.turbulenceFactor * wallAttenuation * energyRamp;
    const dispersion = this.fluid.dispersionSpeed * wallAttenuation * energyRamp;

    const turbulenceX = Math.sin(time + item.phaseX + normalizedY * 2.1) * turbulence;
    const turbulenceY = Math.sin(time * 0.71 + item.phaseY + normalizedX * 1.7) *
      turbulence * 0.3;
    const turbulenceZ = Math.cos(time * 0.89 + item.phaseZ + normalizedY * 1.9) * turbulence;

    return {
      x: tangentX * swirlSpeed * this.swirlDirection * wallAttenuation * energyRamp +
        turbulenceX + item.driftX * dispersion,
      y: upwardBase * item.flowFactor * wallAttenuation * topAttenuation * energyRamp +
        turbulenceY,
      z: tangentZ * swirlSpeed * this.swirlDirection * wallAttenuation * energyRamp +
        turbulenceZ + item.driftZ * dispersion,
    };
  }

  applyFluidForces() {
    const flowActive = this.hasInteracted &&
      this.fluidEnergy > this.fluid.minimumEnergy;

    for (const item of this.bodies) {
      const { body } = item;
      if (!this.hasInteracted) continue;
      if (body.isSleeping() && !flowActive) continue;
      if (flowActive && body.isSleeping()) body.wakeUp();

      const velocity = body.linvel();
      const fluidVelocity = this.getFluidVelocity(item, body.translation(), flowActive);
  ...
  updateFluidState() {
    if (this.fluidEnergy <= 0) return;
    this.fluidAge += this.config.fixedTimeStep;
    this.fluidEnergy *= Math.exp(-this.fluid.flowDecay * this.config.fixedTimeStep);
    if (this.fluidEnergy < this.fluid.minimumEnergy) this.fluidEnergy = 0;
  }
  ...
  enforceSphereBoundary() {
    ...
    for (const item of this.bodies) {
      ...
      const outwardSpeed = velocity.x * this.boundaryNormal.x +
  ...
  update(deltaTime) {
    const maximumAccumulatedTime = this.config.fixedTimeStep * this.config.maxSubSteps;
...
export async function createModelPhysics({ root, camera, canvas, config }) {
  ...
  root.traverse((node) => {
    if (STATIC_BASE_NAME_PATTERN.test(node.name)) {
      node.traverse((child) => {
        if (child.isMesh) staticNodes.add(child);
      });
    }
